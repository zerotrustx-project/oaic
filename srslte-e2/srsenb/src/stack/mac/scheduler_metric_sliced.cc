...
void dl_metric_sliced::set_params(const sched_cell_params_t& cell_params_)
{
  cc_cfg = &cell_params_;
...
void dl_metric_sliced::sched_users(std::map<uint16_t, sched_ue>& ue_db, dl_sf_sched_itf* tti_sched)
{
  tti_alloc = tti_sched;
...
bool dl_metric_sliced::find_allocation(uint32_t min_nof_rbg, uint32_t max_nof_rbg, rbgmask_t* rbgmask)
{
...
dl_harq_proc* dl_metric_sliced::allocate_user(sched_ue* user)
{
...
void ul_metric_sliced::set_params(const sched_cell_params_t& cell_params_)
{
  cc_cfg = &cell_params_;
  log_h  = srslte::logmap::get("MAC ");
}
...
void ul_metric_sliced::sched_users(std::map<uint16_t, sched_ue>& ue_db, ul_sf_sched_itf* tti_sched)
{
  tti_alloc   = tti_sched;
 ...
 */
bool ul_metric_sliced::find_allocation(uint32_t L, prb_interval* alloc)
{
...
ul_harq_proc* ul_metric_sliced::allocate_user_retx_prbs(sched_ue* user)
{
...
ul_harq_proc* ul_metric_sliced::allocate_user_newtx_prbs(sched_ue* user)
{
  if (tti_alloc->is_ul_alloc(user->get_rnti())) {
    return nullptr;
  }
